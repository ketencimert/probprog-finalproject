...
generated quantities {
  ...
  int<lower=0, upper=1> score_ppc[n_game];           // in-sample posterior preds
  int<lower=0, upper=1> score_ppd[n_game_test];      // out-of-sample posterior preds

  // in-sample predictions are conducted iteratively
  for (g in 1:n_game) {
  ...
  // out-of-sample predictions are conducted with latest ratings
  for (g in 1:n_game_test) {
    score_ppd[g] = bernoulli_logit_rng(
