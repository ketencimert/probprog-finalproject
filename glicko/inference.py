...
from glicko.glicko2_inference import Player, evaluate

from glicko.plot import (plot_trace,
                         plot_elbo,
                         plot_loglikelihood,
                         plot_bce
                         )

from glicko.utils import (
    pp_hmc,
    pp_vi,
    pp_map,
    pp_glickman
...
def hmc(glicko_stan, observed_data, gamma_1=2, gamma_2=2):
    ...
    score_pp_mcmc = pp_hmc(samples, observed_data)

    return glicko_mcmc, score_pp_mcmc
...
def meanfield_vi(glicko_stan, observed_data):
    ...
    score_pp_vi = pp_vi(glicko_vi, observed_data)

    return glicko_vi, score_pp_vi
...
def map_opt(glicko_stan, observed_data):
    ...
    score_pp_map = pp_map(glicko_map, observed_data)

    return glicko_map, score_pp_map
...
def glickman(observed_data):
    ...
    score_pp_glickman = pp_glickman(observed_data, ratings_by_time_)

    return score_pp_glickman
